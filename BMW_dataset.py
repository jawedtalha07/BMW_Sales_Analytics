...
models = st.sidebar.multiselect("Select Model(s):", sorted(df["Model"].unique()), default=sorted(df["Model"].unique()))
...
fuel_types = st.sidebar.multiselect("Select Fuel Type(s):", sorted(df["Fuel_Type"].unique()), default=sorted(df["Fuel_Type"].unique()))
transmissions = st.sidebar.multiselect("Select Transmission(s):", sorted(df["Transmission"].unique()), default=sorted(df["Transmission"].unique()))
...
with tab1:
    ...
    if not sales_by_year.empty:
        fig1 = px.line(sales_by_year, x="Yearr", y="Sales_Volume", color="Model", markers=True,  
                       template=template, title="Sales Trends of BMW Models Over Time")
        fig1.update_layout(hovermode="x unified")
        st.plotly_chart(fig1, use_container_width=True)

with tab2:
    st.subheader("Total Price by Model")
    model_price = filtered_df.groupby("Model")["Price_USD"].sum().reset_index().sort_values("Price_USD", ascending=False)
    fig2 = px.bar(model_price, x="Model", y="Price_USD", color="Model", text_auto=".2s", template=template)
...
with tab5:
    ...
    if not num_df.empty:
        corr = num_df.corr()
        fig_corr = ff.create_annotated_heatmap(z=corr.values, x=list(corr.columns), y=list(corr.index),
...
with tab6:
    ...
    fig6 = px.bar(yearly_sales, x="Year", y="YoY Growth (%)", text_auto=".2f", template=template)
    st.plotly_chart(fig6, use_container_width=True)

with tab7:
    st.subheader("Sales by Region (World Map)")
    region_sales = filtered_df.groupby("Region")["Sales_Volume"].sum().reset_index()
    fig7 = px.choropleth(region_sales, locations="Region", locationmode="country names",
                         color="Sales_Volume", color_continuous_scale="Viridis",
                         template=template, title="BMW Global Sales Diistribution")  
    st.plotly_chart(fig7, use_container_width=True)

st.markdown("---")
csv = filtered_df.to_csv(index=False).encode("utf-8")
st.download_button("Download Filtered Data as CSV", data=csv, file_name="filtered_bmw_data.csv", mime="text/csv")
